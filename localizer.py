...
def visualize_loc_from_log(image_dir, query_name, loc, 
                           plot_directory, q, 
                           reconstruction=None,
                           db_image_dir=None, top_k_db=4, dpi=75):
    ...
    for db_idx in db_sort[:top_k_db]:
        ...
        viz.plot_matches(kp_q, kp_db, '#FFFF00', a=0.75)
...
class CameraLocalization:
    ...
    def compute_inlier(self, T, raw_poses, corr_poses, distance_threshold, figure=None):
        ...
        num_inliers = min(10, len(distance_mat))
        num_rows_to_consider = max(len(distance_mat) // 4, 2*num_inliers)
